...
#define STEP_SLOSH			1		// shallow liquid puddle
#define STEP_WADE			2		// wading in liquid
#define STEP_LADDER			3		// climbing ladder
...
void PM_PlayStepSound( int step, float fvol )
{
	...
	if( pmove->multiplayer && ( !g_onladder && hvel.Length() <= 160.0f ))
		return;
...
void PM_CatagorizeTextureType( void )
{
	...
	if( pmove->onground == -1 ) return; // not on ground
...
void PM_WalkMove ()
{
	...
	qboolean  bIsSprint;
	float iSprintFactor = 1.25;

	if ( (pmove->cmd.buttons & IN_RUN) && (pmove->cmd.buttons & IN_FORWARD) )
	{
...
// first try just moving to the destination	
	...
	if (oldonground == -1 &&   // Don't walk up stairs if not on ground.
		(pmove->waterlevel  == 0 || pmove->watertype == CONTENTS_FOG))
		return;
...
//
	...
	if (!pmove->cmd.forwardmove && !pmove->cmd.sidemove && !pmove->cmd.upmove && pmove->watertype != CONTENTS_FLYFIELD) //LRC
...
{
	return ( pmove->waterlevel > 1 && pmove->watertype != CONTENTS_FOG );
}
...
void PM_LadderMove( physent_t *pLadder )
{
	...
	pmtrace_t *tr = pmove->PM_TraceLine( floor, floor, PM_STUDIO_IGNORE, 2, -1 );
...
====================
*/
void PM_NoClip( float factor )
{
	Vector	wishvel;
	float	fmove, smove, umove;
	float	maxspeed = pmove->maxspeed * factor;	// speed for noclip movement
	...
	pmove->forward = pmove->forward.Normalize() * factor;
	pmove->right = pmove->right.Normalize() * factor; 
	pmove->up = pmove->up.Normalize() * factor; 

	// Determine x and y parts of velocity
	wishvel = pmove->forward * fmove + pmove->right * smove + pmove->up * umove;

	Vector wishdir = wishvel.Normalize();
...
void PM_CheckFalling( void )
{
		 ...
		 pmove->flFallVelocity >= PLAYER_FALL_PUNCH_THRESHHOLD )
	{
		...
		if ( fvol > 0.0 )
		{
			...
			float punch = (pmove->flFallVelocity > 700 ? 700 : pmove->flFallVelocity) * 0.42;
		//	pmove->Con_DPrintf("fall vel: %f, punch: %f\n", pmove->flFallVelocity, punch/20);
			pmove->fuser2 += punch;
			pmove->fuser3 += pmove->RandomFloat(punch/-10, punch/10);
			pmove->fuser4 += pmove->RandomFloat(punch/-10, punch/10);
		}
	}
...
void PM_PlayWaterSounds( void )
{
	...
	if  ( ( pmove->oldwaterlevel == 0 && pmove->waterlevel != 0 && pmove->watertype > CONTENTS_FLYFIELD) ||
...
void PM_PlayerMove ( qboolean server )
{
	...
	if (pmove->watertype > CONTENTS_FLYFIELD)
		pmove->oldwaterlevel = pmove->waterlevel;
	else
		pmove->oldwaterlevel = 0;

	// If we are not on ground, store off how fast we are moving down
	if ( pmove->onground == -1 && pmove->movetype != MOVETYPE_NOCLIP )
	{
