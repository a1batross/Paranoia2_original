...
#define TIMING_BEGIN	double _timing_end_, _timing_start_ = gEngfuncs.pfnSys_FloatTime();
#define TIMING_END_STR(S)		\
  _timing_end_ = gEngfuncs.pfnSys_FloatTime();	\
  Msg ("%s: %.3g s\n", S, _timing_end_ - _timing_start_);
#define TIMING_END	TIMING_END_STR(__FUNCTION__)

#define TIMING_INTERMEDIATE(S)						\
  {									\
    double currentTime = gEngfuncs.pfnSys_FloatTime();				\
