...
int CHudSpectator::VidInit()
{
	...
	m_hCrosshair		= SPR_Load("sprites/null.spr");
...
bool CHudSpectator::ParseOverviewFile( )
{
	...
	if (!pfile)
	{
		ALERT( at_aiconsole, "Couldn't open file %s. Using default values for overiew mode.\n", filename );
		return false;
	}
...
void CHudSpectator::InitHUDData()
{
	...
	// reset HUD FOV
	gHUD.m_iFOV = CVAR_GET_FLOAT("default_fov"); // buz: turn off
}
