...
#define ID_BANNER	  	1
#define ID_DONE	  	2
#define ID_CONFIGLABEL	3
#define ID_VIDEOCONFIG	4
#define ID_GAMMA		5
#define ID_GLARE_REDUCTION	6 
#define ID_SIMPLE_SKY	7
#define ID_ALLOW_MATERIALS	8
...
typedef struct
{
	...
	menuAction_s	configLabel;
	menuSpinControl_s	videoConfig;
...
static void UI_VidOptions_GetConfig( void )
{
	...
	// select current video config
	for( int i = 0; i < ARRAYSIZE( videoConfigs ); i++ )
	{
		if( !stricmp( videoConfigs[i], CVAR_GET_STRING( "video_config" )))
		{
			uiVidOptions.videoConfig.generic.name = videoConfigs[i];
			uiVidOptions.videoConfig.curValue = (float)i;
			break;
		}
	}
...
{
	uiVidOptions.videoConfig.generic.name = videoConfigs[(int)uiVidOptions.videoConfig.curValue];
...
static void UI_VidOptions_SetConfig( void )
{
	...
	CVAR_SET_FLOAT( "host_allow_materials", uiVidOptions.hiTextures.enabled );
	CVAR_SET_FLOAT( "gamma", RemapVal( uiVidOptions.gammaIntensity.curValue, 0.0f, 1.0f, 1.8f, 3.0f ));
	CVAR_SET_STRING( "video_config", videoConfigs[(int)uiVidOptions.videoConfig.curValue] );

	char cmd[128];
	sprintf( cmd, "exec scripts/%s.cfg\n", CVAR_GET_STRING( "video_config" ));
...
static void UI_VidOptions_Init( void )
{
	...
	uiVidOptions.configLabel.generic.id = ID_CONFIGLABEL;
	uiVidOptions.configLabel.generic.type = QMTYPE_ACTION;
	uiVidOptions.configLabel.generic.flags = QMF_INACTIVE|QMF_DROPSHADOW|QMF_LEFT_JUSTIFY;
	uiVidOptions.configLabel.generic.name = "Graphics Quality";
	uiVidOptions.configLabel.generic.color = uiColorHelp;
	uiVidOptions.configLabel.generic.charWidth = 12;
	uiVidOptions.configLabel.generic.charHeight = 24;
	uiVidOptions.configLabel.generic.x = 72;
	uiVidOptions.configLabel.generic.y = 218;
	uiVidOptions.configLabel.generic.width = 156;
	uiVidOptions.configLabel.generic.height = 32;

	uiVidOptions.videoConfig.generic.id = ID_VIDEOCONFIG;
	uiVidOptions.videoConfig.generic.type = QMTYPE_SPINCONTROL;
	uiVidOptions.videoConfig.generic.flags = QMF_CENTER_JUSTIFY|QMF_HIGHLIGHTIFFOCUS|QMF_DROPSHADOW|QMF_SMALLFONT;
	uiVidOptions.videoConfig.generic.x = 98;
	uiVidOptions.videoConfig.generic.y = 258;
	uiVidOptions.videoConfig.generic.width = 148;
	uiVidOptions.videoConfig.generic.height = 24;
	uiVidOptions.videoConfig.generic.callback = UI_VidOptions_Callback;
	uiVidOptions.videoConfig.generic.statusText = "Select the optimal perfomance config";
	uiVidOptions.videoConfig.minValue = 0;
	uiVidOptions.videoConfig.maxValue = ARRAYSIZE( videoConfigs ) - 1;
	uiVidOptions.videoConfig.range = 1;
	...
	UI_AddItem( &uiVidOptions.menu, (void *)&uiVidOptions.configLabel );
	UI_AddItem( &uiVidOptions.menu, (void *)&uiVidOptions.videoConfig );
