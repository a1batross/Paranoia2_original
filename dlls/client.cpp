...
void ClientCommand( edict_t *pEntity )
{
	...
	else if ( FStrEq(pcmd, "setblend") ) //buz - test set blending func
	{
		if (CMD_ARGC() != 3)
		{
			ALERT(at_console, "usage: setblend <blend1> <blend2>\n");
			return;
		}
		...
		if (tr.pHit)
		{
			...
			{
				ALERT(at_console, "ok.\n");
				pHitEnt->pev->blending[0] = atoi( CMD_ARGV(1) );
				pHitEnt->pev->blending[1] = atoi( CMD_ARGV(2) );				
				return;
			}
		}

		ALERT(at_console, "no ent on the way.\n");
	}
...
void ClientPrecache( void )
{
	...
	PRECACHE_SOUND("player/gameover.wav");	
...
//	PRECACHE_SOUND("plats/train_use1.wav");		// use a train
	...
	PRECACHE_SOUND( "player/pl_gas_hard.wav" );
...
int AddToFullPack( struct entity_state_s *state, int e, edict_t *ent, edict_t *host, int hostflags, int player, unsigned char *pSet )
{
	...
	if ( ent != host && ent->v.renderfx != 70) // buz: always send 3d skybox ents
	{
		if ( !ENGINE_CHECK_VISIBILITY( (const struct edict_s *)ent, pSet ) )
		{
			if(( ent->v.renderfx == 71 || ent->v.renderfx == 72 ) && ent->v.enemy != NULL )
			{
...
void UpdateClientData ( const struct edict_s *ent, int sendweapons, struct clientdata_s *cd )
{
	...
	cd->iuser3	= MODEL_INDEX( STRING( ent->v.iuser3 ) );
...
{
//	ALERT(at_console, "got usercmd with lightlevel %f\n", cmd->viewangles[2]);
	...
	pl->m_fLightlevel = cmd->viewangles[2] > 1 ? 1 : cmd->viewangles[2]; // buz
