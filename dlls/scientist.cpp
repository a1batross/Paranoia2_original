...
void CScientist::BlockedByPlayer ( CBasePlayer *pBlocker )
{
	...
	{
		PlaySentence( "SC_BLOCKED", 4, VOL_NORM, ATTN_NORM );
	}
}
	...
	case TASK_HEAL:
		...
		else
		{
			if ( TargetDistance() > 90 )
				TaskComplete();
			SetIdealYawAndUpdate( m_hTargetEnt->pev->origin - pev->origin, AI_KEEP_YAW_SPEED );
		}
		break;
	default:
...
//=========================================================
float CScientist :: MaxYawSpeed( void )
{
	float ys;
...
void CScientist :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	case SCIENTIST_AE_NEEDLEON:
		{
			// buz: no model change in heal. Maybe later will be added
...
//	m_flDistTooFar		= 256.0;
	...
	// buz: remove this scientist skin mess etc
	// White hands
/*	pev->skin = 0;
...
void CScientist :: TalkInit()
{
	...
	m_szFriends[3] = "monster_human_military"; // buz
	m_szFriends[4] = "monster_human_alpha"; // buz
	m_szFriends[5] = "monster_alpha_pistol"; // buz
...
{
	// buz
/*	if ((newActivity == ACT_WALK) && m_iUseAlertAnims)
		newActivity = ACT_WALK_SCARED;
	else if ((newActivity == ACT_RUN) && m_iUseAlertAnims)
		newActivity = ACT_RUN_SCARED;*/
