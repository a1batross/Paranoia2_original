...
class CTriggerNoFlashlightZone : public CBaseEntity
{
public:
	void Spawn( void );
	void Touch( CBaseEntity *pOther ) { ClearBits( pOther->pev->effects, EF_DIMLIGHT ); }
};

LINK_ENTITY_TO_CLASS( trigger_noflash, CTriggerNoFlashlightZone );

// Modify an entity's friction
void CTriggerNoFlashlightZone :: Spawn( void )
{
	pev->solid = SOLID_TRIGGER;
	SET_MODEL(ENT(pev), STRING(pev->model));    // set size and link into world
	pev->movetype = MOVETYPE_NONE;
...
TYPEDESCRIPTION	CEnvCustomize::m_SaveData[] = 
{
	...
	DEFINE_FIELD( CEnvCustomize, m_iReflection, FIELD_INTEGER ),
};
...
// ========= buz: paranoia vgui text messages: =============
...
public:
	void Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value );
	void PlayStream( CBasePlayer *pPlayer );
	void KeyValue( KeyValueData *pkvd );
	void Spawn( void );
	void Think( void );
};

LINK_ENTITY_TO_CLASS( trigger_mp3, CAmbientMusic );
...
{
	if( !pev->message )
		pev->frags = 5.0f;	// Paranoia default fade
}

void CAmbientMusic :: PlayStream( CBasePlayer *pPlayer )
{
	...
	{
		CLIENT_COMMAND( pPlayer->edict(), "music \"%s\" \"%s\"\n", STRING( pev->message ), STRING( pev->message ));	// loop
	}
}
...
void CAmbientMusic :: Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	{
		PlayStream( (CBasePlayer *)UTIL_PlayerByIndex( i ));
	}
}
...
class CTriggerGoal : public CBaseEntity
{
public:
	void Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
	{
		...
		pPlayer->PlayerSetGoalDesc(pev->message, pev->netname, pev->noise);
	}
...
//	ALERT(at_console, " CStartRush got succes report\n");
	...
	{
		FireTargets( STRING(pev->netname), who, this, USE_TOGGLE, 0 );
...
#define	SF_GASAREA_STARTOFF	1
#define	GASAREA_THINKTIME	0.3

class CGasArea : public CBaseEntity
{
public:
	void Spawn()
	{
		...
		if ( FBitSet(pev->spawnflags, SF_GASAREA_STARTOFF) )
		{
			m_iEnabled = FALSE;
			SetThink(NULL);
			pev->nextthink = -1;
		...
		{
			m_iEnabled = TRUE;
			SetThink(&CGasArea::DamageThink);
			pev->nextthink = gpGlobals->time + GASAREA_THINKTIME;
		}
	}
	...
	{
		Vector warnmin = pev->absmin - Vector(m_fWarningDist, m_fWarningDist, 0);
		Vector warnmax = pev->absmax + Vector(m_fWarningDist, m_fWarningDist, 0);

		// find all monsters in volume
		CBaseEntity *pList[32];
		int count = UTIL_EntitiesInBox( pList, 32, warnmin, warnmax, FL_MONSTER|FL_CLIENT );

		for (int i = 0; i < count; i++)
		{
			CBaseMonster* pMonster = pList[i]->MyMonsterPointer();
			if ( pMonster && pMonster->pev->takedamage != DAMAGE_NO && pMonster->IsAlive() )
			{
				// is monster in gas area, or just walking nearby?
				if (pMonster->pev->origin.x > pev->absmin.x &&
					pMonster->pev->origin.y > pev->absmin.y &&
					pMonster->pev->origin.z > pev->absmin.z &&
					pMonster->pev->origin.x < pev->absmax.x &&
					pMonster->pev->origin.y < pev->absmax.y &&
					pMonster->pev->origin.z < pev->absmax.z )
				{
					// inflict damage
					pMonster->TakeDamage( pev, pev, m_fDamage, DMG_NERVEGAS );
					pMonster->GasWarning(1);
				}
				else
				{
					// just warn him
					pMonster->GasWarning(0);
				}
			}
		}
	...
	void KeyValue( KeyValueData *pkvd )
	{
		if (FStrEq(pkvd->szKeyName, "warndist"))
		{
			m_fWarningDist = atof(pkvd->szValue);
			pkvd->fHandled = TRUE;
		}
		if (FStrEq(pkvd->szKeyName, "gasdamage"))
		{
...
private:
	int		m_iEnabled;
	float	m_fWarningDist;
	float	m_fDamage;
};

LINK_ENTITY_TO_CLASS( trigger_gasarea, CGasArea );

TYPEDESCRIPTION	CGasArea::m_SaveData[] = 
{
	DEFINE_FIELD( CGasArea, m_iEnabled, FIELD_INTEGER ),
	DEFINE_FIELD( CGasArea, m_fWarningDist, FIELD_FLOAT ),
	DEFINE_FIELD( CGasArea, m_fDamage, FIELD_FLOAT ),
};

IMPLEMENT_SAVERESTORE(CGasArea,CBaseEntity);