...
//LRC	Vector		m_vecControllerUsePos;
	...
	float		m_flBulletDamage; // 0 means use Bullet type's default damage
...
//LRC	DEFINE_FIELD( CFuncTank, m_vecControllerUsePos, FIELD_VECTOR ),
	...
	DEFINE_FIELD( CFuncTank, m_flBulletDamage, FIELD_FLOAT ),
...
void CFuncTank :: KeyValue( KeyValueData *pkvd )
{
	...
	{
		m_flBulletDamage = atof(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
		...
		if ( bulletCount > 0 )
		{
			for ( i = 0; i < bulletCount; i++ )
			{
				...
				case TANK_BULLET_9MM:
					if( !m_flBulletDamage ) m_flBulletDamage = gSkillData.monDmg9MM;
					FireBullets( 1, barrelEnd, forward, gTankSpread[m_spread], 4096.0f, BULLET_NORMAL, m_flBulletDamage, pevAttacker );
					break;
				case TANK_BULLET_MP5:
					if( !m_flBulletDamage ) m_flBulletDamage = gSkillData.monDmgMP5;
					FireBullets( 1, barrelEnd, forward, gTankSpread[m_spread], 4096.0f, BULLET_NORMAL, m_flBulletDamage, pevAttacker );
					break;
				case TANK_BULLET_12MM:
					if( !m_flBulletDamage ) m_flBulletDamage = gSkillData.monDmg12MM;
					FireBullets( 1, barrelEnd, forward, gTankSpread[m_spread], 4096.0f, BULLET_NORMAL, m_flBulletDamage, pevAttacker );
					break;
...
//	ALERT(at_console,"controls %p triggered by \"%s\" %p\n", this, STRING(pCaller->pev->classname), pCaller);
	...
	if ( !m_pController && useType != USE_OFF )
	{
		...
		if (m_active)
		{
			...
			{
				m_pController->m_pActiveItem->Holster( true );
