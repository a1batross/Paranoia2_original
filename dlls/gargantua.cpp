...
void CGargantua::StompAttack( void )
{
	...
	if ( trace.flFraction < 1.0 )
		UTIL_TraceCustomDecal( &trace, "gargstomp", RANDOM_FLOAT( 0.0f, 360.0f ));
}
	...
	for ( i = 0; i < 2; i++ )
	{
		if ( m_pFlame[i] )
		{
			...
			if ( trace.flFraction != 1.0 && gpGlobals->time > m_streakTime )
			{
				...
				streaks = TRUE;
				UTIL_TraceCustomDecal( &trace, "smallscorch", RANDOM_FLOAT( 0.0f, 360.0f ));
			}
...
//=========================================================
float CGargantua :: MaxYawSpeed( void )
{
	float ys;
