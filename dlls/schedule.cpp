...
void CBaseMonster :: RunTask ( Task_t *pTask )
{
	...
	case TASK_PLAY_SEQUENCE_FACE_TARGET:
		{
			...
			if ( pTarget )
			{
				SetIdealYawAndUpdate( pTarget->pev->origin - pev->origin , AI_KEEP_YAW_SPEED );
			}
	...
	case TASK_WAIT_PVS:
		{
			// g-cont. TESTTEST. completely ignore PVS
			if ( 1 )//	!FNullEnt(FIND_CLIENT_IN_PVS(edict())) )
			{
				TaskComplete();
			}
			break;
...
void CBaseMonster :: SetTurnActivity ( void )
{
	...
	if( flYD <= -80 && flYD >= -100 && LookupActivity( ACT_90_RIGHT ) != ACTIVITY_NOT_AVAILABLE )
	{
		// 90 degree right.
	...
	if( flYD >= 80 && flYD <= 100 && LookupActivity( ACT_90_LEFT ) != ACTIVITY_NOT_AVAILABLE )
	{
		// 90 degree left.
		Remember( bits_MEMORY_TURNING );
	...
	if( fabs( flYD ) >= 160 && LookupActivity ( ACT_180_LEFT ) != ACTIVITY_NOT_AVAILABLE )
	{
		Remember( bits_MEMORY_TURNING );
...
//				return;
	...
	case TASK_FACE_LASTPOSITION:
		SetIdealYawToTarget( m_vecLastPosition );
		SetTurnActivity(); 
		break;
	...
	case TASK_FACE_TARGET:
		if ( m_hTargetEnt != NULL )
		{
			SetIdealYawToTarget( m_hTargetEnt->pev->origin );
			SetTurnActivity(); 
		}
		else
			TaskFail();
	...
	case TASK_FACE_ENEMY:
		{
			SetIdealYawToTarget( m_vecEnemyLKP );
			SetTurnActivity(); 
			break;
		}
	case TASK_FACE_IDEAL:
	...
	case TASK_FACE_ROUTE:
		{
			...
			{
				SetIdealYawToTarget( m_Route[m_iRouteIndex].vecLocation );

				if( fabs( DeltaIdealYaw() ) <= 15.0f )
				{
	...
	case TASK_FACE_SCRIPT:
		{
			if ( m_pCine != NULL && m_pCine->m_fMoveTo != 0) // movetype "no move" makes us ignore turntype
			{
				...
				{
				case 0:
					SetIdealYaw( UTIL_AngleMod( m_pCine->pev->angles.y ));
					break;
				case 1:
					...
					if (m_hTargetEnt)
						...
						SetIdealYawToTarget( m_pCine->pev->origin );
					break;
				// default: don't turn
				}
			}
	...
	case MONSTERSTATE_ALERT:
		{
			...
			{
				if ( fabs( DeltaIdealYaw() ) < (1.0 - m_flFieldOfView) * 60 ) // roughly in the correct direction
				{
					return GetScheduleOfType( SCHED_TAKE_COVER_FROM_ORIGIN );
				}
				else if ( LookupActivity( ACT_SMALL_FLINCH ) != -1 )
				{
