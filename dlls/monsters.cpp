...
//	DEFINE_FIELD( CBaseMonster, m_afSoundTypes, FIELD_INTEGER ),
	...
	DEFINE_FIELD( CBaseMonster, m_vecFacingDir, FIELD_VECTOR ),
	DEFINE_FIELD( CBaseMonster, m_vecDirection, FIELD_VECTOR ),
	...
	DEFINE_FIELD( CBaseMonster, m_hRushEntity, FIELD_STRING ), // buz
	DEFINE_FIELD( CBaseMonster, m_iRushMovetype, FIELD_INTEGER ), // buz
	DEFINE_FIELD( CBaseMonster, m_flRushDistance, FIELD_FLOAT ), // buz
	DEFINE_FIELD( CBaseMonster, m_iUseAlertAnims, FIELD_INTEGER ), // buz	
};
...
void CBaseMonster :: Look ( int iDistance )
{
	...
	if ( !FBitSet( pev->spawnflags, SF_MONSTER_PRISONER ) )
	{
		...
		for ( int i = 0; i < count; i++ )
		{
			...
			{
				// buz
				if (!FStringNull(pSightEnt->pev->noise3) && !FStringNull(pev->targetname))
				{
					if (FClassnameIs(pSightEnt->pev,"monster_target"))
					{
						if (!FStrEq(STRING(pev->targetname), STRING(pSightEnt->pev->noise3)))
							continue;
					}
				}
...
void CBaseMonster :: SetActivity ( Activity NewActivity )
{
	...
	// NOTE: set yaw_speed right when activity is changed
	if ( iSequence > ACTIVITY_NOT_AVAILABLE )
		RecalculateYawSpeed();
}

void CBaseMonster :: SetIdealActivity( Activity NewActivity )
{
...
//#define	LOCAL_STEP_SIZE	8 // buz test
...
//	DrawRoute( pev, m_Route, m_iRouteIndex, 0, 200, 0 );
	...
	if (!pev->gaitsequence) // buz: dont turn strafing monsters
	{
...
//				ALERT ( at_aiconsole, "Couldn't Triangulate\n" );
				...
				else
				{
					...
					// buz: if blocker is player, tell him to move away from my path!
...
//	WALK_MOVE( ENT(pev), flYaw, m_flGroundSpeed * flInterval, WALKMOVE_NORMAL );
	...
	// buz: strafing monsters movement
	if (pev->gaitsequence)
	{
		float flTotal = m_flGroundSpeed * flInterval;
		float flStep;
		while (flTotal > 0.001)
		{
...
//	if (m_iStartAlert)
//		m_IdealMonsterState	= MONSTERSTATE_ALERT; // buz
	...
	// buz: force body selection
	if (pev->impulse)
	{
		pev->body = pev->impulse;
		pev->impulse = 0;
	}
}
...
//		SetIdealYawToTargetAndUpdate( m_pGoalEnt->pev->origin );
...
void CBaseMonster :: StartMonster ( void )
{
	...
	if ((LookupActivity ( ACT_TWITCH ) != ACTIVITY_NOT_AVAILABLE) &&
		 !FClassnameIs(pev,"monster_leech")) // hack: leech uses ACT_TWITCH animation
	{
...
int CBaseMonster::IRelationship ( CBaseEntity *pTarget )
{
	...
	{			 //   NONE	MACH	PLYR	HPASS	HMIL	AMIL	APASS	AMONST	APREY	APRED	INSECT	PLRALY	PBWPN	ABWPN	FACT_A	FACT_B	FACT_C	TERR
	/*NONE*/		{ R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO},
	/*MACHINE*/		{ R_NO,	R_NO,	R_DL,	R_DL,	R_NO,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_NO,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL},
	/*PLAYER*/		{ R_NO,	R_DL,	R_NO,	R_NO,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_NO,	R_NO,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL},
	/*HUMANPASSIVE*/{ R_NO,	R_NO,	R_AL,	R_AL,	R_HT,	R_FR,	R_NO,	R_HT,	R_DL,	R_FR,	R_NO,	R_AL,	R_NO,	R_NO,	R_DL,	R_DL,	R_DL,	R_DL},
	/*HUMANMILITAR*/{ R_NO,	R_NO,	R_HT,	R_DL,	R_NO,	R_HT,	R_DL,	R_DL,	R_DL,	R_DL,	R_NO,	R_HT,	R_NO,	R_NO,	R_DL,	R_DL,	R_DL,	R_DL},
	/*ALIENMILITAR*/{ R_NO,	R_DL,	R_HT,	R_DL,	R_HT,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_DL,	R_NO,	R_NO,	R_DL,	R_DL,	R_DL,	R_DL},
	/*ALIENPASSIVE*/{ R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_NO,	R_DL,	R_DL,	R_DL,	R_DL},
	...
	/*ALIENPREDATO*/{ R_NO,	R_NO,	R_DL,	R_DL,	R_DL,	R_NO,	R_NO,	R_NO,	R_HT,	R_DL,	R_NO,	R_DL,	R_NO,	R_NO,	R_DL,	R_DL,	R_DL,	R_DL},
	/*INSECT*/		{ R_FR,	R_FR,	R_FR,	R_FR,	R_FR,	R_NO,	R_FR,	R_FR,	R_FR,	R_FR,	R_NO,	R_FR,	R_NO,	R_NO,	R_FR,	R_FR,	R_FR,	R_DL},
	/*PLAYERALLY*/	{ R_NO,	R_DL,	R_AL,	R_AL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_NO,	R_NO,	R_NO,	R_NO,	R_DL,	R_DL,	R_DL,	R_DL},
	...
	/*FACTION_A*/   { R_NO,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_NO,	R_DL,	R_DL,	R_DL,	R_AL,	R_DL,	R_DL,	R_DL},
	/*FACTION_B*/   { R_NO,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_NO,	R_DL,	R_DL,	R_DL,	R_DL,	R_AL,	R_DL,	R_DL},
	/*FACTION_C*/   { R_NO,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_NO,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_AL,	R_DL},
	/*TERRORISTS*/  { R_NO,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_NO,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_DL,	R_AL} // buz
	};
...
//-----------------------------------------------------------------------------
void CBaseMonster :: AddFacingTarget( CBaseEntity *pTarget, float flImportance, float flDuration, float flRamp )
{
	m_facingQueue.Add( pTarget, flImportance, flDuration, flRamp );
}

void CBaseMonster :: AddFacingTarget( const Vector &vecPosition, float flImportance, float flDuration, float flRamp )
{
	m_facingQueue.Add( vecPosition, flImportance, flDuration, flRamp );
}

void CBaseMonster :: AddFacingTarget( CBaseEntity *pTarget, const Vector &vecPosition, float flImportance, float flDuration, float flRamp )
{
	m_facingQueue.Add( pTarget, vecPosition, flImportance, flDuration, flRamp );
}

float CBaseMonster :: GetFacingDirection( Vector &vecDir )
{
	...
	for( i = 0; i < m_facingQueue.Count(); i++ )
	{
		...
		Vector tmp = (m_facingQueue[i].GetPosition() - pev->origin).Normalize();

		vecDir = vecDir * (1.0f - flInterest) + tmp * flInterest;

		flTotalInterest = (1.0f - (1.0f - flTotalInterest) * (1.0f - flInterest));
...
// ideal_yaw
//=========================================================
float CBaseMonster :: CalcIdealYaw( const Vector &vecTarget )
{
	Vector	vecProjection = g_vecZero;
	...
	else if ( m_movementActivity == ACT_STRAFE_RIGHT )
	{
		...
		return UTIL_VecToYaw( vecProjection - pev->origin );
	}
	else
	{
		vecProjection = vecTarget;
	}

	m_vecDirection = (vecProjection - pev->origin);
	m_vecFacingDir = m_vecDirection;

	return UTIL_VecToYaw( m_vecDirection );
}
...
//=========================================================
float CBaseMonster :: DeltaIdealYaw( void )
{
...
//=========================================================
// ClampYaw - turns a monster towards its ideal_yaw
//=========================================================
float CBaseMonster :: ClampYaw( float yawSpeedPerSec, float current, float target, float time )
{
	if( current != target )
	{
...
void CBaseMonster :: UpdateYaw( int yawSpeed )
{
	...
	newYaw = ClampYaw( (float)yawSpeed * 10.0, current, ideal, gpGlobals->frametime );
...
float CBaseMonster :: MaxYawSpeed( void )
{
	switch( m_Activity )
	{
	case ACT_TURN_LEFT:
...
//-----------------------------------------------------------------------------
float CBaseMonster :: CalcYawSpeed( void )
{
	float maxYaw = MaxYawSpeed();

	if( m_movementGoal != MOVEGOAL_NONE && m_flGroundSpeed != 0.0 )
	{
		// ---------------------------------------------------
		...
		if( m_flGroundSpeed > 0 ) 
		{
			...
			// -----------------------------------------------------------------
			float waypointDist = (m_Route[m_iRouteIndex].vecLocation - pev->origin).Length();

			// If waypoint is close, aim for the waypoint
			if (waypointDist < 100)
			{
				float scale = 1.0f + (0.01f * (100.0f - waypointDist));
				return (maxYaw * scale);
			}
		}
	}
...
void CBaseMonster :: RecalculateYawSpeed( void )
{
	SetYawSpeed( CalcYawSpeed() ); 
}

void CBaseMonster :: SetIdealYawToTarget( const Vector &target )
{
	SetIdealYaw( CalcIdealYaw( target ));
}

void CBaseMonster :: SetIdealYawToTargetAndUpdate( const Vector &target, float yawSpeed )
{
	SetIdealYawAndUpdate( CalcIdealYaw( target ), yawSpeed ); 
}

void CBaseMonster :: SetIdealYawAndUpdate( float idealYaw, float yawSpeed )
{
...
void CBaseMonster :: CalcFacing( float flInterval )
{
	float currentYaw = UTIL_AngleMod( pev->angles.y );
	float goalYaw = UTIL_VecToYaw( m_vecFacingDir );
	float deltaYaw = fabs( UTIL_AngleDiff( goalYaw, currentYaw ) );

	if( deltaYaw > 15 )
	{
		float speed = deltaYaw * 4.0f; // i.e., any maneuver takes a quarter a second
		float clampedYaw = ClampYaw( speed, currentYaw, goalYaw, flInterval );

		if( clampedYaw != goalYaw )
		{
			m_vecFacingDir = UTIL_YawToVector( clampedYaw );
		}
	}
}

void CBaseMonster :: MoveFacing( void )
{
	// required movement direction
	float flMoveYaw = UTIL_VecToYaw( m_vecDirection );

	float fSequenceMoveYaw = GetSequenceMoveYaw( pev->sequence );
	if( fSequenceMoveYaw == NOMOTION )
		fSequenceMoveYaw = 0;

	if( !HasPoseParameter( pev->sequence, "move_yaw" ))
	{
	...
	else
	{
		...
		dir = m_vecFacingDir * (1.0f - flInfluence) + dir * flInfluence;
		dir = dir.Normalize();
		...
		float flDiff = UTIL_AngleDiff( flMoveYaw, pev->angles.y );

		// HACKHACK: temporary fixup negative oscillations until this will do properly
		if( pev->ideal_yaw == pev->angles.y )
			flDiff = 0.0f;
#ifdef _DEBUG
		Msg( "ideal_yaw %g angles.y: %g, move_yaw %g\n", GetIdealYaw(), pev->angles.y, flDiff );
#endif
...
{ 
	...
	{	
		surf = TRACE_SURFACE( ENT( pEntity->pev ), pev->origin + Vector( 0, 0, 8 ), pev->origin - Vector( 0, 0, 16 ));
		pMaterial = COM_MatDefFromSurface( surf, ptr.vecEndPos );
	}
	else if( pEntity->pev->solid == SOLID_CUSTOM )
	{
		if( ptr.pMat ) pMaterial = ptr.pMat->effects;
	}

	if( pMaterial )
	{
		// fill sound array
		for( cnt = 0; pMaterial->step_sounds[cnt] != NULL; cnt++ )
			rgsz[cnt] = (char *)pMaterial->step_sounds[cnt];
	}
	...
	EMIT_SOUND_DYN( ENT( pev ), CHAN_BODY, rgsz[RANDOM_LONG(0, cnt - 1)], fvol, ATTN_NORM, 0, 96 + RANDOM_LONG( 0, 0xf ));
}
...
//		ALERT(at_console, "trigger cond set to %d\n", m_iTriggerCondition);
		pkvd->fHandled = TRUE;
	}
...
{
	if ( fabs( DeltaIdealYaw() ) <= 0.006 )//!!!BUGBUG - no magic numbers!!!
	{
		return TRUE;
	}
...
class CTargetMonster : public CBaseEntity
{
public:
	void Spawn( void );
...
LINK_ENTITY_TO_CLASS( monster_target, CTargetMonster );

void CTargetMonster :: Spawn ( void )
{
...
void CTargetMonster::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
