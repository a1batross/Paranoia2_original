...
void CAmbientGeneric :: Precache( void )
{
	...
	if ( m_fActive )
	{
		if (m_pPlayFrom || FBitSet(pev->spawnflags, AMBIENT_SOUND_FROM_PLAYER))
		{
...
void CAmbientGeneric :: StartPlayFrom( void )
{
	...
	if (FBitSet(pev->spawnflags, AMBIENT_SOUND_FROM_PLAYER)) // buz
	{
		CBaseEntity *pPlayer = CBaseEntity::Instance(g_engfuncs.pfnPEntityOfEntIndex(1));
		EMIT_SOUND_SUIT( pPlayer->edict(), szSoundFile);
	}
	else
	{
		EMIT_SOUND_DYN( m_pPlayFrom, m_iChannel, szSoundFile, //LRC
...
void CAmbientGeneric :: RampThink( void )
{
	...
	if (m_dpv.spinup || m_dpv.spindown)
	{
		...
		if (pitch < m_dpv.pitchstart)
		{
			...
			else if (FBitSet(pev->spawnflags, AMBIENT_SOUND_FROM_PLAYER)) // buz
			{
				CBaseEntity *pPlayer = CBaseEntity::Instance(g_engfuncs.pfnPEntityOfEntIndex(1));
				STOP_SOUND( pPlayer->edict(), CHAN_STATIC, szSoundFile);
			}
			else
			{
				UTIL_EmitAmbientSound(ENT(pev), pev->origin, szSoundFile, 
...
int SENTENCEG_PlayRndSz(edict_t *entity, const char *szgroupname, 
					  float volume, float attenuation, int flags, int pitch, int channel)
{
	char name[64];
	int ipick;
	int isentenceg;
...
{
	// TODO: parse matspec.txt here, precache sounds etc
}
