//
//                 Quake1 Model Viewer (c) 1999 by Mete Ciragan
//
// file:           pakviewer.cpp
...
PAKViewer::handleEvent (mxEvent *event)
{
	...
	case mxEvent::Action:
	{
		...
		case IDC_PAKVIEWER: // tvPAK
			...
			{
				pmMenu->setEnabled (1, strstr (d_currLumpName, ".spr") != 0);
...
PAKViewer::OnPAKViewer ()
{
	...
	if (tvi)
	{
		...
		if (!d_loadEntirePAK)
		{
			// finally insert "sprites/"
			strcpy (tmp, d_currLumpName);
			strcpy (d_currLumpName, "sprites/");
			strcat (d_currLumpName, tmp);
		}
	}
...
PAKViewer::OnLoadTexture (int pos)
{
	...
	// now load the things
	if (g_GlWindow->loadTexture (str2, pos))
	{
		if (pos == TEXTURE_BACKGROUND)
			g_SPRViewer->setShowBackground (true);
		else
			g_SPRViewer->setShowGround (true);
		g_GlWindow->redraw ();
	}
	else
...
PAKViewer::openPAKFile (const char *pakFile)
{
	...
	for (int i = 0; i < numLumps; i++)
	{
		if (d_loadEntirePAK || !strncmp (lumps[i].name, "sprites", 7))
		{
			char *tok;
			if (d_loadEntirePAK)
				tok = &lumps[i].name[0];
